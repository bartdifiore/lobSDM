...
write_rds(fit_bd, "Data/Derived/model_fitbd")
...
nd <- df_mod2 %>% 
  modelr::data_grid(depth = mean(depth, na.rm = T), 
                    bt_seasonal = modelr::seq_range(bt_seasonal, 100),
                    year_season_numeric = median(year_season_numeric, na.rm = T), 
                    survey = unique(survey))

p <- predict(fit_bd, newdata = nd, se_fit = F, re_form = , model = NA, type = "response")
...
nd <- df_mod2 %>% 
  modelr::data_grid(depth = mean(depth, na.rm = T), 
                    bt_seasonal = mean(bt_seasonal, na.rm = T),
                    year_season_numeric = year_season_numeric, 
                    survey = unique(survey))

p <- predict(fit_bd, newdata = nd, se_fit = F, re_form = NA, model = NA, type = "response")


ggplot(p, aes(year_season_numeric, est))+
  geom_line(aes(color = survey)) +
  # geom_ribbon(aes(group = survey), alpha = 0.4) +
  scale_x_continuous() +
  coord_cartesian(expand = F) +
