...
Finally, the ephemeral spatio-temporal variation component with an AR1 auto-regressive structure to facilitate predicting this component to future time steps. Notably, when we implemented this model structure with an ar1 process on the spatio-temporal variation, we ran into gradient issues immediately. Fitting with a RW removed the error, though generated a warning about the Hessian matrix, signaling issues with resolving the uncertainty around fixed and random effects. It is still a bit unclear to me why this is happening, other than there is something going on with time_varying for both the intercepts and the spatio-temporal terms?

While trouble shooting that, one option might be looking for ways of simplifying the `time_varying` component of the model depending on the importance of that AR1 process?
```{r}
# Extract AR1 coefficient for intercepts
fit_sp$sd_report
pl <- as.list(fit_sp$sd_report, "Estimate")
pls <- as.list(fit_sp$sd_report, "Std. Error")
...
*I think* that is lending to support to modeling the intercepts as IID (rather than a constant or fixed effect, which would then somewhat limit our capacity to make forecasts/projections?) I gave that a shot and it works, also seemingly generating a better model (AIC 52637 vs 54505 baseline model and then log likelihood of -26297 vs. -27236 baseline model). I then looked at the values and estimates and they are incredibly small values with virtually no variance. With that additional information, thinking we might be able to drop the "year" component entirely from the model.
```{r}
